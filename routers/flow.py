...
async def _generate_paraphrase_response(session: ConversationSession, user_input: str, openai_service: OpenAIService, is_tts_enabled: Optional[bool]) -> tuple[str, List[LearnWord], Optional[str]]:
    ...
    prompt = f"""
        ...
        Construct 2 short sentences (16-22 words) in {session.from_lang} and replace 6 separate words with {session.to_lang}.
        Learned_expressions: all 6 words replaced with {session.to_lang} should be in learned_expressions.
        The first sentence must paraphrase the user's key idea. Always keep the conversation going.        
